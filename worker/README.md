...
| `demo.mjs [file.json] [--list]` | Replays the whole story from `demo.json` into whichever registry is configured: the Sepolia collision and lifecycle, then real loans on Ethereum mainnet from NFTfi and Blend. `demo-mainnet.json` holds the mainnet half alone. The source transactions already exist and are already final, so the sequence runs in minutes rather than hours. Each step may name its own `sourceChainId` and `sourceRpc`. |
...
carries NFTfi v3 and Blend alongside the native shape. If the two halves ever disagree, the
